class SearchApiModel {
  List<Data_searchApi>? data;
  bool? error;
  String? statusCode;
  String? message;

  SearchApiModel({this.data, this.error, this.statusCode, this.message});

  SearchApiModel.fromJson(Map<String, dynamic> json) {
    if (json['data'] != null) {
      data = <Data_searchApi>[];
      json['data'].forEach((v) {
        data!.add(new Data_searchApi.fromJson(v));
      });
    }
    error = json['error'];
...
class Data_searchApi {
  ...
  Data_searchApi.fromJson(Map<String, dynamic> json) {
    id = json['id'];
